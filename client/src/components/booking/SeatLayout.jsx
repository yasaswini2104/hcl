 ...
 * Renders a seat grid.
 * Props:
 *   seats: array of { id, label, status: AVAILABLE | BOOKED }
 *   selectedIds: array of seat ids
 *   onToggle: (seatId) => void
 *   columnsPerRow: number (default 4) — visual columns; aisle drawn at midpoint
 */
            ...
            {row.map((seat, colIdx) => (
              <>
                <button
                  ...
                  className={`h-10 w-10 md:h-12 md:w-12 rounded-md text-xs font-semibold transition ${seatStyle(
                    seat
                  )}`}
                ...
                {colIdx === aisleAfter - 1 && (
                  <span key={`aisle-${rowIdx}`} className="w-4 md:w-6" />
                )}
              </>
            ))}
          </div>
